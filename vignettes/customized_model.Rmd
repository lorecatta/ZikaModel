...
Here we will demonstrate how run a customized model which simulates general vector control. There are three parameters which simulate general vector control:
...
Running a customized model requires to use a different set of functions which will be demonstrated here. We will use `create_generator()` to return a function that will generate an instance of the model and use that instance to run the model with user-defined parameters. The function `create_generator()` has an argument `...` which passes arguments to `model_param_list_create()` thus overriding the defaults values of the model parameters. 
